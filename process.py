...
def process_csv(input_file_path, output_file_path):
    ...
    with open('./output_data/temp.csv', 'w') as file: 
        file.write(data) # use temp file for safety first
    ...
    df = df.map(lambda x: re.sub(r'\s*,\s*', ',', str(x)))
    ...
    df = df.map(lambda x: re.sub(r',(?=[^\s])', ', ', str(x))) # adds a space after a comma if it is inside the columns
